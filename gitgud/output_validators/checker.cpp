...
ll checkAns(InStream& stream, TResult bad_verdict, ll n) {
    ll cost = 0;
    ll k = stream.readLong((ll)0, 1e6, "k");
    ll x = 1e6 + 1;
    ...
    for (ll i = 1; i <= k; i++) {
        ll y = stream.readLong((ll)1, 1e6, "y");
        ll l = stream.readLong((ll)1, 1e6, "l");
        cost += l;
        if (y > x) cost += PENALTY;
    ...
    if (cost > MAXC) {
        stream.quitf(bad_verdict, "cost is %lld > %d", cost, MAXC);
    }

    if (st.size() > 1) {
        stream.quitf(bad_verdict, "failed to reach skill n");
    }
...
int main(int argc, char* argv[]) {
    ...
    ll cost = checkAns(ouf, _wa, n);
    checkAns(ans, _fail, n);
    ...
    quitf(_ok, "tests passed, cost = %lld", cost);
}
