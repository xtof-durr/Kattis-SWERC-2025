...
#define pb push_back
#define _ << ' ' <<
#define $ << fixed << setprecision(10) <<
#define $$ _ fixed << setprecision(10) <<
    ...
    for(int TT=0;TT<t;TT++){
      ...
      while (!is_answer) {
	      ...
	      if(is_answer){
	        for(int i=0;i<n;i++){
	          for(int j=i;j<n;j++){
	            int Resp1=ouf.readInt(-1,40);
	            int Resp2=bitlen(p[i]^p[j+1]);
	            if(Resp1!=Resp2){
	                tout << "Wrong answer at interval [" << i + 1 << ", " << j + 1 << "]: guessed" _ Resp1 << ", answer" _ Resp2 << nf;
	                cout << -2 << nf;
	                quitf(_wa, "Wrong guess");
	            }
	          }
	        }
	        tout << "Guessed correctly, used" $$ Penalty _ "penalty and" _ queries _ "queries" << nf;
	      }else{
	          cout<<bitlen(p[u]^p[v+1])<<endl<<flush;
	      }
	  }
    }
    quitf(_ok, "All guesses correct");
}
