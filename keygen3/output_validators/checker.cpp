...
// true ⇔ a[0 … n-1] is bitonic as (previously) defined in the statement
static bool isBitonic(vector<int> a) {
    reverse(a.begin(), a.end());
    for (auto &u : a) u = (int)a.size() - u + 1;
    int n = (int)a.size();
    for (int pivot = 0; pivot < n; ++pivot) {
        bool ok = true;
        /* non-increasing prefix [1 … pivot] */
        for (int j = 1; j <= pivot && ok; ++j)
            if (a[j - 1] < a[j]) ok = false;

        /* non-decreasing suffix [pivot … n-2] */
        for (int j = pivot; j + 1 < n && ok; ++j)
...
// number of disjoint cycles in permutation p (1-based values, 0-based indices)
static int cycleCount(const vector<int>& p) {
    int n = (int)p.size(), cnt = 0;
    vector<char> vis(n, 0);
    for (int i = 0; i < n; ++i)
        if (!vis[i]) {
            int v = i;
            do { vis[v] = 1; v = p[v] - 1; } while (!vis[v]);
            ++cnt;
        }
    return cnt;
}

/* ---------- per-testcase validator ---------- */

static int checkAnswer(InStream& st,
                        TResult bad,
                        int n, int m)
{
    int k = st.readInt(1, 2000, "r");
    vector<vector<int>> perms;
    perms.reserve(k);

    for (int id = 1; id <= k; ++id) {
        /* read one permutation */
        vector<int> p(n);
        vector<char> used(n + 1, 0);

        for (int i = 0; i < n; ++i) {
            p[i] = st.readInt(1, n, "p");
            if (used[p[i]])
                st.quitf(bad,
                         "permutation #%d: value %d appears twice",
                         id, p[i]);
            used[p[i]] = 1;
        }
        ...
        /* property checks */
        if (!isBitonic(p))
            st.quitf(bad, "permutation #%d is not bitonic", id);

        int c = cycleCount(p);
        if (c != m)
            st.quitf(bad,
                     "permutation #%d has %d cycles, expected %d",
                     id, c, m);
    ...
    /* distinctness */
    if (set<vector<int>>(perms.begin(), perms.end()).size() != (size_t)k)
        st.quitf(bad, "permutations are not distinct");
...
int main(int argc, char* argv[])
{
    ...
    int n = inf.readInt();
    int m = inf.readInt();                 // required number of cycles

    int k_out = checkAnswer(ouf, _wa,   n, m);      // contestant
    int k_ans = checkAnswer(ans, _fail, n, m);      // jury/reference

    if (k_out < k_ans) quitf(_wa, "permutations missing");
    if (k_out > k_ans) quitf(_fail, "permutations missing on jury solution");
