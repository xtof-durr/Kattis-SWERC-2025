...
/* ---------- helper: turn a vector<int> into a hashable key ---------- */
static string key(const vector<int>& v)
{
    string s;
    s.reserve(v.size() * 4);
    for (int x : v) {
...
/* ---------- core routine: validate one output stream ---------- */
static void checkAnswer(InStream& st,
                        TResult   bad_verdict,
                        int       n,
                        int       m,
                        const vector<vector<int>>& pieces)
{
    const int L  = n * m / 2;                // length of hidden permutation
    const int TL = n * m;                    // total length of concatenation

    /* ---- read permutation p_1 … p_L ---- */
    vector<int> p(L + 1, 0);                 // 1-based values
    ...
    for (int i = 0; i < L; ++i) {
        ...
        if (used[v])
            st.quitf(bad_verdict,
                     "value %d appears twice in the permutation", v);
        used[v] = 1;
        p[i + 1] = v;
    }

    /* ---- build the concatenation [p | p] ---- */
    vector<int> big(TL);
    for (int i = 0; i < L; ++i) big[i]      = p[i + 1];
    for (int i = 0; i < L; ++i) big[L + i]  = p[i + 1];

    /* ---- split into n consecutive blocks of length m ---- */
    map<string,int> need;                    // multiset of required blocks
    for (int blk = 0; blk < n; ++blk) {
        vector<int> v;
        v.reserve(m);
        for (int j = 0; j < m; ++j)
            v.push_back(big[blk * m + j]);
        need[key(v)]++;
    }

    /* ---- multiset of available input pieces ---- */
    map<string,int> have;
    ...
    if (need != have) {
        // find first mismatch for a nicer message
        for (const auto& [k,c] : need) {
            int h = have[k];
            if (c != h) {
                if (h < c)
                    st.quitf(bad_verdict,
                             "block %s missing %d occurrence(s)",
                             k.c_str(), c - h);
                else
                    st.quitf(bad_verdict,
                             "block %s appears %d extra time(s)",
                             k.c_str(), h - c);
            }
        }
        st.quitf(bad_verdict,
                 "blocks mismatch (should be impossible to reach here)");
    }
}
...
int main(int argc, char* argv[])
{
    ...
    for (int tc = 1; tc <= t; ++tc) {
        ...
        const int L = n * m / 2;
        vector<vector<int>> pieces(n, vector<int>(m));

        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < m; ++j) {
                pieces[i][j] = inf.readInt(1, L, "a_ij");
            }
        }

        /* check contestant output */
        checkAnswer(ouf, _wa,   n, m, pieces);

        /* check jury/reference output */
        checkAnswer(ans, _fail, n, m, pieces);
    }

    if (!ouf.seekEof())
        ouf.quitf(_pe, "extra output");
    if (!ans.seekEof())
        ans.quitf(_fail, "extra output on jury solution");
